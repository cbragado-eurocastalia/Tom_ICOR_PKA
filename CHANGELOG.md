...
- **`DERIVATIVES.md`** at the root: the general rules for building and distributing your own version of the scaffold. It is written as rules that apply to everyone identically, not as permission granted case by case, and it covers the five places a derivative goes wrong quietly:
  ...
  4. **The platform-terms trap.** A course platform's standard "do not copy, do not share" clause sitting over CC BY-SA files is an additional restriction under § 2(a)(5)(C) and terminates the licence automatically under § 6(a). § 6(b)(1) reinstates it as of the date you cure, provided you cure within 30 days of discovering the breach, and reinstatement does not affect any remedy we may have for the period of the breach. A plain login wall is fine. It is the clause that breaches, not the wall.
  5. **The update check must be re-pointed or turned off.** See below.

- **`SECURITY.md`** at the root. The scaffold had no security policy. It states supported versions, the reporting channel, response targets and coordinated-disclosure expectations, what is in and out of scope, exactly what the scaffold does over the network, the standing release-blocking distribution rules (bring-your-own-key, secrets by reference, release integrity), and a safe-harbour statement for good-faith research.

  **The reporting channel is GitHub private vulnerability reporting, and only that.** No email address is published. Our general support address is a product-support channel with an automated assistant in front of it: it is not access-controlled for security material, and an exploit report is the last thing that should land there. The document says so explicitly rather than leaving a reporter to guess. An email fallback returns in a later release only once there is an address confirmed to reach a human on a channel that does not auto-reply.

  **Response times are stated as targets, not commitments.** We aim to acknowledge within 5 working days and to assess within 10, with a 90-day disclosure target, and the document says plainly that these are targets a small team works to rather than a contractual response time. It also tells a reporter what to do if we go quiet. A published response guarantee nobody is staffed to meet is worse than an honest target, and this file is copied into derivatives whose maintainers have less capacity than we do.
...
- **Cockpit version drift in the manifest.** `expansions.items[mypka-cockpit].expansion_yaml_version` read `1.2.1` against a shipped Cockpit of `1.5.1`. Because the updater compares that declared value to decide whether an installed Expansion is behind, the stale entry produced a wrong "your cockpit is behind" notice in both directions. Now `1.5.2`, matching the Cockpit released alongside this scaffold.

- **The Cockpit mislabelled its own licence, in the one file that is machine-read.** `Expansions/mypka-cockpit/expansion.yaml` declared `license: PolyForm-Noncommercial-1.0.0`. The Cockpit is not under that licence. It is under the **myICOR Cockpit Personal-Use License v1.0**, which is *based on* PolyForm Noncommercial 1.0.0 and adds attribution conditions and express AS-IS, liability and financial-data disclaimers that the standard text does not contain. Someone relying on the declared identifier could have argued the standard text governed and the added conditions never applied. The identifier is now `LicenseRef-myICOR-Cockpit-Personal-Use-1.0`, the correct SPDX form for a licence that is not on the SPDX list, and the Expansion spec uses this as its worked example of labelling an adapted licence. **Nothing about what you may do with the Cockpit has changed.** Shipped as **Cockpit 1.5.2**, because the shipped manifest is hash-pinned and a pin is one hash for one version; rewriting the 1.5.1 pin would have made every correctly-installed 1.5.1 Cockpit look tampered with. See `Expansions/mypka-cockpit/CHANGELOG.md`.
...
- **The updater's root-containment guard now resolves symlinks.** `scripts/update-scaffold.py` checked containment with `os.path.normpath`, which collapses `..` but follows symlinks, so a symlink planted at an allow-listed path inside the scaffold could redirect a write outside the scaffold root while the guard reported the path as contained. It now resolves with `os.path.realpath` on **both** the root and the candidate path, and fails closed under `--apply`. Resolving both sides is the part that matters: resolving only the candidate would have refused every update for every member on macOS (where `/tmp` is a real symlink) or with a symlinked home directory, turning a containment bug into a total denial of the update path. `..` traversal was always refused and still is. Reaching the original issue required write access to your scaffold already, so it granted nobody anything new; it is fixed because `SECURITY.md` invites researchers to attack exactly that boundary and the guarantee should be true.

- **Release CI now guards the declared version SSOT, not only its mirror.** The pipeline checked the `VERSION` file against the tag. `manifest.json` `scaffold_version` is the authoritative version and is what `check-version.py` reads, so a drifted manifest would have passed CI while misreporting every member's installed version and therefore their update state. `manifest.json` is now checked against the tag first, failing closed if the file is missing, if `scaffold_version` is absent or empty, or if it disagrees, with the mirror checked after it.
...
- **The updater's user-state refusal is applied to the path an update bundle declares, not to the path the write resolves to.** Root containment resolves symlinks and is sound (see Fixed, above). The allow-list and user-state checks do not, so a symlink planted inside your own scaffold at an allow-listed path can still redirect a write into user state, and the plan prints "0 of your files touched" while it happens, because that line is fixed text rather than a computed count. Reaching this requires write access to your scaffold already, and anyone holding that can write to `PKM/` directly and more easily, so it grants nobody anything they do not have. It is an honesty failure in a safety control rather than a way in. **Three fixes are queued for 5.4.1:** a fourth gate that re-tests the resolved path against the same two lists, gated on the resolved path actually differing from the declared one so that members with no symlink see byte-identical behaviour; backups filed under the resolved path rather than the declared one, so recovery lands where you would look for it; and that hardcoded count either computed or dropped. `SECURITY.md` states the guarantee against the declared path until those land.

- **Changes to `Expansions/docs/expansion-spec.md` do not reach existing folders.** The path matches the `Expansions/*/**` user-state glob, which the updater refuses ahead of the framework allow-list, so the spec cannot be listed in `changes.paths` without failing the whole update closed. That gate winning is correct behaviour rather than a bug: `Expansions/*/**` is where your installed Expansion state lives, and the rule that the scaffold updater never writes there is worth more than documentation reach. The spec is an authoring contract that binds pack authors when they sit down to write a pack, which is a fetch-the-current-spec moment, so a resident older copy changes nobody's rights or obligations. The compliance-relevant content is in `LICENSE-MAP.md` and `DERIVATIVES.md`, both root files, both delivered. If the spec is to reach existing folders it will move to a numbered Guideline rather than the updater being weakened.
